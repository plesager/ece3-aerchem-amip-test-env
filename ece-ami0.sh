...
# -----------------------------------------------------------------------------
config="ifs:cosp amip oasis tm5:chem,o3fb,ch4fb,aerfb"

#  Use initial conditions and restart files from a previous experiment
prev_exp_name=ECE3
prev_exp_root=/ec/res4/hpcperm/nm6/ece3data/ifs/T255L91
prev_exp_date=19700101
init_from_restart_path=${prev_exp_root}/${prev_exp_date}
...
output_control_files_dir=${start_dir}/ctrl/cmip6-output-control-files-pextra/CMIP/EC-EARTH-AerChem/cmip6-experiment-CMIP-amip
...
# restart files
nem_restart_file_path=${init_from_restart_path}
...
then
    ...
    tm5_restart_file_path=${init_from_restart_path}
...
case "${ifs_grid}" in
    ...
    T511L* ) has_config nemo && oas_mb_fluxcorr=1.07945 ;;
...
do
    ...
    then
        ...
        ${grib_set} -s dataDate=$(date -u -d "$run_start_date" +%Y%m%d) \
            ${init_from_restart_path}/ICMGG${prev_exp_name}INIUA \
                                                            ICMGG${exp_name}INIUA
        ${grib_set} -s dataDate=$(date -u -d "$run_start_date" +%Y%m%d) \
            ${init_from_restart_path}/ICMSH${prev_exp_name}INIT \
                                                            ICMSH${exp_name}INIT
        ${grib_set} -s dataDate=$(date -u -d "$run_start_date" +%Y%m%d) \
            ${init_from_restart_path}/ICMGG${prev_exp_name}INIT \
                                                            ICMGG${exp_name}INIT
	    ...
	    . ${ctrl_file_dir}/domain_def_nemo.xml.sh > domain_def_nemo.xml
            ...
            then
                ...
                then
                    ...
                    case $(echo ${ifs_cmip6_scenario} | tr '[:upper:]' '[:lower:]') in
                        ...
                        ssp1-1.9*) pis_scen="ssp119"; pis_sco2_mfix="IMAGE-ssp119-1-2-1";;
        ...
        then
            ...
            case ${tm5_istart} in
                33|32) ln -s ${tm5_restart_file_path}/TM5_restart_${prev_exp_date}_0000_glb300x200.nc \
                          TM5_restart_${leg_start_date_yyyymmdd}_0000_glb300x200.nc
                    ;;
        ...
        then
            ...
            do
                if ! has_config tm5:keeploc && [[ ${f:0:2} == 'r_' ]]
                then
                    # strip loc000* fields from restart files for tm5
                    # set tm5:keeploc to keep the loc000* fields
                    cdo delvar,loc* ${init_from_restart_path}/$f $f
                else
                    cp ${init_from_restart_path}/$f .
                fi
            done
    ...
    then
        ...
        do
            ...
            mkdir -p ${run_dir}/run${n}/output/downscaling
        done
